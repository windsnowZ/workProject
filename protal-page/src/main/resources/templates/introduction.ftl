												...
												<div class="theme-signin-left">
													...
													<div class="theme-options">
														...
														<dd>
															...
															<input v-model="cartInfo.buyNum" id="text_box" name="" type="text"  style="width:30px;" />
			...
			<script>
				...
				var vm = new Vue({
					...
					data:{
						...
						cartInfo:{
							skuId:"",
							buyNum:1,//int   "10"  //string
						}
					},
					methods:{
						chooseSpec(spec/*口味*/, opt/*原味*/){//用户选择规格
						...
						addCart(){//加购
							this.cartInfo.buyNum = parseInt(this.cartInfo.buyNum);
							wfx.http.post("/cart/add",this.cartInfo).then(res=>{
								if(res.data == "invalid token"){//token不合法，空
									//跳转登录页
									location.href = "http://passport.wfx.com?redirect="+location.href
								}else{
									if(res.success){//业务成功
										//购物车列表页
										location.href = "http://cart.wfx.com";
									}else{
										alert(res.msg);
									}
								}
